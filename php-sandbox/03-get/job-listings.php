...
function calculateAverageSalary($listings)
{
  ...
  function filterListingsByLocation($listings, $location) {
    ...
    // The filterListingsByLocation function takes two arguments: $listings, which is an array of job listings, and $location, which is the location you want to filter the job listings by. The function uses array_filter to iterate over each job listing in $listings and applies a callback function to check if the location of each job matches the given $location. If the location matches, the job listing is included in the filtered array

  };

  // Check if location query param is there
